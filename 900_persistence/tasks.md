...
| T-030 | Implementar el commit por etapa en los skills de etapa (`ingest`/`interview`/`discovery`/`prototype-protocol`) y un bootstrap de repo git al inicio del estadio, conforme a `principles.md:37` y `methodology.md:515` (L-009) | pendiente (siguiente paso, junto a T-034/T-035) | alta |
...
| T-032 | Conservar el `client_brief.md` de reciclaje como fixture permanente versionado en `Base_Harness` (ubicación por decidir con el humano) | pendiente (el humano completa los archivos del proyecto de prueba) | alta |
...
| T-034 | G1 — Leer `git log` al iniciar sesión en `startup-protocol` (E1, E10) | pendiente | alta |
| T-035 | G5 — Aplicar la convención de commits `tipo(<incremento>): descripción` del Apéndice de `methodology.md` en `closing-protocol` Paso 6.3 | pendiente | alta |
...
| T-041 | Alinear la semántica de `cubierta` para §7 entre `ingest-protocol` (métrica+umbral) y `discovery-protocol` (métrica+umbral+método) (L-011) | pendiente | media |
...
- [ ] T-030 — Implementar el **commit por etapa** en los skills de etapa (`ingest-protocol`, `interview-protocol`, `discovery-protocol`, `prototype-protocol`) y un **bootstrap de repo git** al inicio del estadio de Prototipo, conforme a `principles.md:37` y `methodology.md:515` §7. Hoy solo `closing-protocol` toca git, y un proyecto puede recorrer el descubrimiento completo sin control de versiones sin que nadie lo detecte ([[lessons]] L-009). **Siguiente paso inmediato**, junto a T-034 (G1) y T-035 (G5) — la zona git, de una pasada — y bloqueante de la repetición de T-027.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-009, [[tasks]] T-034, T-035
- [ ] T-034 — G1: leer `git log` al iniciar sesión en `startup-protocol` (E1, E10). Hoy el protocolo de inicio no verifica el estado real del repositorio ni el historial reciente, pese a que E1/E10 lo exigen. Bloqueante de la repetición de T-027 (zona git, de una pasada junto a T-030/T-035).
      Prioridad: alta · Responsable: — · Ref: [[tasks]] T-030, T-035, [[progress]]
- [ ] T-035 — G5: aplicar la convención de commits `tipo(<incremento>): descripción` del Apéndice de `methodology.md` en el Paso 6.3 de `closing-protocol`. Hoy el mensaje de commit no sigue ninguna convención fija. Bloqueante de la repetición de T-027 (zona git, de una pasada junto a T-030/T-034).
      Prioridad: alta · Responsable: — · Ref: [[tasks]] T-030, T-034, [[progress]]
