...
| T-048 | Añadir `Bash` a `onboarding-reader`, `onboarding-interviewer` y `onboarding-writer`: sus skills exigen commit y no pueden ejecutarlo (L-017). Bloqueante real de L-009 | pendiente | alta |
...
| T-050 | Check de conformidad que detecte salida de etapa sin commit, en vez de seguir reescribiendo la regla (L-019, tercera reaparición de L-009). Se apoya en T-039 | pendiente | alta |
...
| T-052 | Actualizar el criterio de aceptación de L-005 en `T-027_procedimiento.md`: quedó desfasado frente a T-044 (un nombre con procedencia en `project.yaml` no es fabricación) | pendiente | media |
...
| T-054 | Conjunto de actores a construir como campo cerrado del `discovery.md`, obedecido por `prototype-protocol` en vez de su invariante "solo el generador" (L-022) | pendiente | alta |
...
| T-057 | Capa barata de conformidad sobre artefactos + `git log` (sin motor de traza): captura 8 de los 10 hallazgos de las corridas 1-3. Precede y desbloquea parcialmente a T-039 | pendiente | alta |
...
- [ ] T-048 — Añadir `Bash` a las definiciones de `onboarding-reader`, `onboarding-interviewer` y `onboarding-writer` (hoy `Read, Write, Edit, Glob, Grep, Skill`), o bien hacer que sus skills deleguen el commit explícitamente en el orquestador. **Es el bloqueante de fondo de L-009:** mientras estos agentes no puedan commitear, el commit por etapa lo suple el orquestador a mano y el mecanismo **nunca se ejercita** en la prueba — sus commits no cuentan como evidencia (regla de L-016). Verificar de paso que ningún otro agente del molde declare herramientas incoherentes con su skill.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-017, L-009, L-016, [[tasks]] T-027, T-050
...
- [ ] T-050 — Check de conformidad que detecte **salida de etapa sin commit**: comparar los artefactos de etapa presentes contra el árbol trackeado y señalar cualquier etapa materializada que no tenga commit. Motivo (L-019): la regla textual ya es correcta desde T-030/T-042 y aun así el fallo reapareció por tercera vez; corregir el prompt otra vez no es una corrección, es una repetición. Se apoya en el motor de traza de T-039.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-019, L-009, L-013, [[tasks]] T-039, T-042, T-027
...
- [ ] T-052 — Actualizar el criterio de aceptación de L-005 en `T-027_procedimiento.md` §3: hoy exige que el nombre aparezca **literalmente en el brief**, criterio escrito antes de T-044. Con `project.yaml` como insumo declarado, un nombre tomado de allí y con procedencia anotada es correcto, no fabricación — la corrida 3 lo hizo bien y el criterio lo marcaría como fallo. Redefinir sobre **procedencia declarada y verificable**, no sobre literalidad en un archivo concreto.
      Prioridad: media · Responsable: — · Ref: [[lessons]] L-005, L-015, [[tasks]] T-044, T-033, T-027
...
- [ ] T-054 — Declarar el **conjunto de actores a construir** como campo cerrado y explícito del `discovery.md` (p. ej. *"Actores a construir: generador, operador"*), y hacer que `prototype-protocol` **obedezca ese campo** en vez de afirmar por su cuenta "solo el generador" (líneas 45-46, 73 y el invariante de la 136). Motivo (L-022): hoy el discovery y el skill responden distinto a la misma pregunta, y por eso dos corridas construyeron alcances distintos siendo ambas "correctas". Una sola fuente de verdad. Reabre parcialmente T-022.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-022, L-011, [[tasks]] T-022, T-055, T-027
- [ ] T-055 — Contrato verificable en el borde **discovery→prototipo**: el constructor emite un reporte que mapea cada **regla dura y criterio del Gatekeeper (§7)** a la evidencia concreta de que se cumple, antes de ceder el gate. Motivo (L-020): las reglas duras ya están declaradas en el discovery y **nadie comprueba** que el prototipo las cumpla — en la corrida 3 la violación (cuarta solicitud sin asignar) apareció solo al ejercitar el artefacto a mano. Convierte "el agente decidió cuánto construir" en "el discovery declaró qué construir y un check lo verifica". Solapa con T-051; decidir si se funden.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-020, L-022, [[tasks]] T-051, T-050, T-039
...
- [ ] T-057 — **Capa barata de conformidad** sobre `artefactos + git log`, sin motor de traza. Hallazgo que la justifica: de los 10 hallazgos de las corridas 1-3, **8 eran detectables mecánicamente sin traza** — procedencia del nombre y estado `n/a` (leer el artefacto), doble escritura de la confirmación y commit por etapa (leer `git log`), coherencia agente↔skill en herramientas (lint estático de las definiciones). Solo L-018 (contenido perdido) y L-020 (comportamiento del motor) necesitan más. Construir esta capa primero da la mayor parte del valor de E13/§10 a una fracción del costo del motor de traza (T-039), que queda para los checks del tipo "¿leyó antes de escribir?".
      Prioridad: alta · Responsable: — · Ref: [[tasks]] T-039, T-050, T-031, [[decisions]] D-023, [[lessons]] L-017, L-019
