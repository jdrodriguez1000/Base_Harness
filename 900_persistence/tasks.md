...
| T-041 | Alinear la semántica de `cubierta` para §7 entre `ingest-protocol` (métrica+umbral) y `discovery-protocol` (métrica+umbral+método) (L-011) | pendiente (confirmada empíricamente en la corrida 2: §7 quedó `cubierta` sin método — sube de prioridad) | alta |
| T-042 | Desacoplar el commit de etapa del gate humano en `ingest-protocol` y `discovery-protocol`: el disparador es la salida de etapa, no la aprobación; si el artefacto está en borrador se confirma igual con `[sin confirmar]` en el mensaje (L-013). Reabre parcialmente T-030 | pendiente | alta |
| T-043 | Contrato de entrada entre etapas: cada skill verifica `Estado`/`Confirmado` del artefacto que consume y AVISA (no bloquea) si viene en borrador (L-014) | pendiente | alta |
| T-044 | `project.yaml` como insumo declarado desde el Paso 0 de los skills de etapa: metadatos de proyecto (fuente `project.yaml`) vs. contenido del cliente (fuente brief, `<no declarado>` si falta); todo valor lleva su procedencia (L-015) | pendiente | alta |
| T-045 | Checkpoints intra-etapa con commit (y push según decisión pendiente): commit eager en unidades naturales (bloque de preguntas, iteración del bucle), no por cada Q&A | pendiente (decisión de push abierta, ver A-003) | alta |
| T-046 | Corregir el criterio de aceptación de L-007 en `T-027_procedimiento.md` para que "nunca se confirmó" cuente también como fallo (L-016) | pendiente | media |
...
- [ ] T-042 — Desacoplar el **commit de etapa** del gate humano en `ingest-protocol` y `discovery-protocol`: el disparador debe ser la **salida de la etapa**, no la aprobación. Si el artefacto está en borrador, se confirma igual y el mensaje lo indica (p. ej. `[sin confirmar]`). Perder trazabilidad no puede ser el precio de saltarse un gate legítimo. Reabre parcialmente T-030. Detectado en la corrida 2 de T-027 (reaparición de L-009).
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-013, [[tasks]] T-030, T-027
- [ ] T-043 — **Contrato de entrada entre etapas:** cada skill verifica el `Estado`/`Confirmado por el humano` del artefacto que consume y **AVISA** si viene en borrador (avisa, no bloquea — la autoridad es del humano, NC-6). Hoy `interview-protocol` consume un extracto en borrador sin detectarlo.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-014, [[tasks]] T-027
- [ ] T-044 — `project.yaml` como **insumo declarado** desde el Paso 0 de los skills de etapa, separando dos clases de campo: metadatos del proyecto (nombre, repo, rama) que vienen de `project.yaml` como fuente declarada, frente a contenido del cliente que viene SOLO del brief con cita textual y `<no declarado>` si falta. Condición verificable: **todo valor lleva su procedencia** (p. ej. `Proyecto: ReciclApp (fuente: _context/project.yaml)`). Preserva la regla anti-fabricación de T-028 y habría impedido que el interviewer atribuyera mal la fuente de un dato. Petición explícita del humano.
      Prioridad: alta · Responsable: — · Ref: [[lessons]] L-015, L-005, [[tasks]] T-028, T-027
- [ ] T-045 — **Checkpoints intra-etapa con commit** (y push según decisión pendiente): el commit por etapa no basta — una entrevista larga o un bucle de prototipado pueden ser horas dentro de una sola etapa; si la sesión se cae sin cerrarla se pierde todo. Checkpoints en unidades naturales (bloque de preguntas, iteración del bucle), NO por cada Q&A. Eager, no "al suspender". Un push fallido informa y no bloquea. Petición explícita del humano. **Decisión pendiente:** si los checkpoints empujan al remoto respetando `auto_push` o se quedan locales y solo el cierre empuja (ver [[assumptions]] A-003).
      Prioridad: alta · Responsable: — · Ref: [[assumptions]] A-003, [[tasks]] T-027
- [ ] T-046 — Corregir el criterio de aceptación de L-007 en `T-027_procedimiento.md` para que "nunca se confirmó" cuente también como fallo (hoy solo contempla "se confirmó antes de tiempo" y da por bueno un caso donde el gate no se ejecutó nunca).
      Prioridad: media · Responsable: — · Ref: [[lessons]] L-016, [[tasks]] T-027
...
- [~] T-027 — Prueba end-to-end del flujo COMPLETO de prototipado, con el caso de reciclaje como fixture: `client_brief` → `onboarding-reader` (`document_extract.md`) → `onboarding-interviewer` solo-huecos (`interview_document.md`) → `onboarding-writer` (`discovery.md`) → `prototype-builder` (`prototype/`), todo dentro de `_prototype/` (D-028). Engloba y encadena T-024 y T-026. Modalidad observador (D-029): la sesión del harness no ejecuta la prueba, actúa como validador.
      ...
      **Antes de la corrida 3:** aplicar T-042 (desacoplar commit de gate), T-043 (contrato de entrada entre etapas), T-044 (`project.yaml` con procedencia) y T-045 (checkpoints intra-etapa), y resolver la decisión de push en checkpoints (A-003). La precondición `git init` debe cumplirse antes de invocar al primer agente.
      Prioridad: alta · Responsable: — · Ref: [[progress]], [[decisions]] D-026, D-027, D-028, D-029, [[lessons]] L-005…L-009, L-013…L-016, [[tasks]] T-042, T-043, T-044, T-045, T-046
